...
"""
    Zach Chan
    主要是实现了三种机器学习分类器的比较，分别是：

        - 神经网络 (MLPClassifier)
        - 支持向量机 (SVM)
        - 偏最小二乘回归分析 (PLSRegression)
        - 代码中对于每种分类器定义了一个函数，比如：
            ANN(X_train, X_test, y_train, y_test, StandScaler=None)，
            这个函数中会训练模型，使用测试集进行预测，并计算准确率。
    
    代码还使用了一些预处理模块，如StandardScaler进行数据标准化，pd.get_dummies()将分类标签转换为独热编码。
"""
