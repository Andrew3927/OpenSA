...
"""
    这段代码主要实现了一个函数Simcalculation，用来计算原始光谱数据与
    目标光谱数据的相似度。可以计算SAM(Spectral Angle Mapper)，
    SID(Spectral Information Divergence)，
    HsiSam(Hsi Spectral Angle Mapper)，
    mssim(Multi-scale Structural Similarity)和
    mpsnr(Multi-scale Peak Signal to Noise Ratio)五种不同的相似度
    计算方法。
"""
